...
## 결정 사항
- **D1 (#1 라이브 추종):** store handle을 드래그마다 변형(approach A: skipWireRecalc=false)하지 않고,
  SyncEngine에서 blocks/junctions dirty면 wires도 dirty 처리(approach B). 이유:
  - WireRenderer가 엔드포인트를 블록 위치에서 라이브 해석하므로, 재렌더만 하면 추종됨.
  - approach A는 매 프레임 recalc+simplify로 user handle 드리프트/비용 위험.
  - 핸들(꺾임)은 드래그 중 고정→릴리즈에서 기존 recalc로 정리(표준 rubber-band).
  - 대안(approach A)은 자동라우팅 wire의 직각 라이브 유지에 유리하나 위험이 커서 보류.
