...
- 놓쳤다 잡은 결합: mapping.rs의 `default_for_tag`(sim TagDefinition, 프로덕션
  ...
  pub로 승격(크로스-크레이트).
- **wasm 게이트**: `cargo check -p opcua-codec --target wasm32-unknown-unknown`은
  opcua-codec 소스만으론 wasm-clean(std::time/net/fs·chrono·uuid·tokio 직접 의존
  전무). 그러나 의존 modone-contract가 uuid::v4/chrono::now를 쓰므로 wasm 빌드엔
  contract에 `uuid:["v4","js"]` + `chrono:["serde","wasmbind"]` 피처가 필요.
  이 2개 피처만 추가하면 게이트 green 확인. 코드 변경은 불필요.
  → 계약 §3(wasm-purity)은 워크트리 A 소관이라 이 피처 추가는 별도 커밋으로
  분리·플래그(추가/네이티브 무해). A가 ID/시간 주입을 택하면 대체 가능.
