...
///
/// This is separate from [`super::address_space::OpcUaAccessLevel`] because the
...
impl OpcUaMappingConfig {
    ...
    ///
    /// Uses [`super::address_space::is_bool_address`] to detect whether the
    ...
    pub fn default_for_address(address: CanonicalAddress) -> Self {
        let is_bool = super::address_space::is_bool_address(address);
        Self::default_from_is_bool(is_bool)
    }

    /// Auto-generates a default mapping config for a tag definition.
    ///
    /// Inspects the tag's `canonical_address` to determine whether the
    /// underlying register is boolean, and produces the appropriate default
    /// mapping (Boolean or UInt16, wordCount=1, BigEndian, ReadOnly).
    pub fn default_for_tag(tag: &crate::sim::types::TagDefinition) -> Self {
        Self::default_for_address(tag.canonical_address)
    }
...
pub struct RegisterRange {
    ...
    pub area: crate::plc_runtime::CanonicalAreaKind,
...
mod tests {
    ...
    fn default_for_tag_bool_tag() {
        use crate::sim::types::{RuntimeBinding, TagAccessLevel, TagClass, TagDefinition};
        ...
        let cfg = OpcUaMappingConfig::default_for_tag(&tag);
        assert_eq!(cfg.opcua_data_type, OpcUaDataType::Boolean);
        assert_eq!(cfg.word_count, 1);
        assert_eq!(cfg.byte_order, ByteOrder::BigEndian);
        assert_eq!(cfg.access_level, MappingAccessLevel::ReadOnly);
    ...
    fn store_synchronized_deletion_simulation() {
        // Simulates the tag deletion + mapping removal workflow
        ...
        // Step 1: Create a tag and its mapping config
        let tag = registry
        ...
        // Step 2: Delete tag — both registry and store must be cleaned
        registry.remove("pump-speed").unwrap();
        store.remove("pump-speed");

        // Step 3: Verify both are cleaned up
        assert!(!store.contains("pump-speed"));
    ...
    fn store_batch_deletion_simulation() {
        // Simulates batch tag deletion + mapping removal
        use crate::sim::tag_registry::TagRegistry;
        use crate::sim::types::RegisterTagRequest;
        ...
        // Create multiple tags with mapping configs
        for (id, area, idx) in [
            ...
            let tag = registry
                ...
                .unwrap();
            store.insert(tag.tag_id.clone(), OpcUaMappingConfig::default_for_tag(&tag));
        }
        assert_eq!(store.len(), 3);

        // Batch delete tags 1 and 3
        ...
        // Verify: tag-2 survives, others gone
        assert_eq!(store.len(), 1);
